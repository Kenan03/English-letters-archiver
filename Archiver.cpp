...
{
	string q;
	for (int i = 0; i < code.length(); i++) //проходим по строке и выводим 0 и 1
	{
		char c = code[i];
...
{
	for (int i = 0; i < code1.length(); i++) //проходим по строке и выводим 0 и 1
	{
		if (code1[i] == '0')
		{
			code2.push_back(0);
...
void Archiver::coding(const string& filename1, vector <bool>& code2)
{
	...
	int count = 0;
	char buf = 0; // 00000000    010101  e->101010       01010111 
	int j = 0;
